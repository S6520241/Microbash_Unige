	...
	while (tmp) {
		...
		if (*tmp=='<') {
			...
			if (*tmp=='$') {
				...
				/*** TO BE DONE START ***/
				tmp = getenv(tmp + 1); // skip the '$'
    			if (!tmp) tmp = "";    // fallback to empty string
				/*** TO BE DONE END ***/
			}
...
check_t check_redirections(const line_t * const l)
{
	...
	/*** TO BE DONE START ***/
	for (int i = 0; i < l->n_commands; ++i) {
		const command_t *cmd = l->commands[i];

		if (cmd->in_pathname && i != 0) {
			fprintf(stderr, "Redirection error: input redirection allowed only in the first command.\n");
			return CHECK_FAILED;
		}

		if (cmd->out_pathname && i != (l->n_commands - 1)) {
			fprintf(stderr, "Redirection error: output redirection allowed only in the last command.\n");
			return CHECK_FAILED;
		}
	}
	/*** TO BE DONE END ***/
...
check_t check_cd(const line_t * const l)
{
	 ...
	 */
	/*** TO BE DONE START ***/
	command_t *cmd = l->commands[0];

	if (strcmp(cmd->args[0], CD) == 0) {
		//1.
		if (l->n_commands != 1) {
			fprintf(stderr, "Error: 'cd' must be the only command in the line.\n");
			return CHECK_FAILED;
		}
		//2.
		if (cmd->in_pathname || cmd->out_pathname) {
			fprintf(stderr, "Error: 'cd' cannot have input/output redirection.\n");
			return CHECK_FAILED;
		}
		//3.
		if (cmd->n_args != 2) {
...
void redirect(int from_fd, int to_fd)
{
	...
	if (from_fd != NO_REDIR) {
		if (dup2(from_fd, to_fd) == -1)
			fatal_errno("redirect: dup2 failed");
		if (close(from_fd) == -1)
			fatal_errno("redirect: close failed");
	}
	/*** TO BE DONE END ***/
}
...
void run_child(const command_t * const c, int c_stdin, int c_stdout)
{
	...
	pid_t pid = fork();
	if (pid < 0)
		fatal_errno("run_child: fork failed");
